describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
     ...
     const shawdow_root = await product_item.getProperty('shadowRoot');
     const button = await shawdow_root.$('button');
     await button.click();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (const item of prodItems) {
      ...
      const shadowElem = await shadowRoot.asElement();
    ...
    const cart_elemt = await page.$('#cart-count');
    const cart_inner = await cart_elemt.getProperty('innerText');
    const cart_count = await cart_inner.jsonValue();
    expect(parseInt(cart_count)).toBe(20);
  }, 25000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cart_inner = await cart_elemt.getProperty('innerText');
    const cart_count = await cart_inner.jsonValue();
    expect(parseInt(cart_count)).toBe(0);
